...
export default function TopRatedSection({ savedMovies, toggleMyList }) {
  ...
  return (
    <section className="px-2 py-[5px] md:px-16 md:py-[30px]">
      <h3 className="text-white text-[20px] font-bold md:text-[32px]">Top Rating Film dan Series Hari ini</h3>
      ...
      <div className="relative group">
        ...
        <div className="flex gap-4 overflow-x-auto scroll-smooth scrollbar-hide py-6" id="topSlider" ref={sliderRef} style={{ scrollbarWidth: 'none' }}>
          {topRatedData.map((movie) => {
            const isFavorited = savedMovies.some((item) => item.title === movie.title);

            return (
              <div key={movie.id} className="relative min-w-[245px] h-[340px] shrink-0" onMouseLeave={() => !isMobile && setExpandedId(null)}>
                ...
                >
                  ...
                  <span className="absolute top-2 right-4 text-4xl font-bold text-white drop-shadow-md opacity-80">{movie.id}</span>
                </div>

                {expandedId === movie.id && (
                  <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-[340px] h-[350px] bg-[#181A1C] rounded-xl shadow-2xl overflow-hidden scale-110 transition-all duration-300 border border-gray-700">
        ...
        <button
          className="absolute right-[-25px] top-1/2 -translate-y-1/2 z-20 bg-black/50 hover:bg-black/80 text-white p-3 rounded-full hidden md:flex items-center justify-center transition"
